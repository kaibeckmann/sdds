#!/bin/bash
if [[ "$#" -lt  4 ]]; then
    echo "usage: $0 <msg_count> <max_msg_size> <start_size> <mbps>"
    exit
fi
...
for (( size=$start_size; size<=$max_msg_size; size=$size*2 )); do
    ...
    log="rti_throughput_"$msg_count"_"$size".log"
    ssh $echo -l pi './throughput_pub '$mbps' '$size' > rti_print_pub_'$msg_count'_'$size'.log' & 
    ssh $host -l pi './throughput_sub '$msg_count' '$log' > rti_print_sub_'$msg_count'_'$size'.log'  

    ssh $echo -l pi 'killall throughput_pub'
done

cd ../
eval_dir="eval_rti_throughput_"$msg_count"_"$max_msg_size"_"$(date +%s)
echo "mkdir $eval_dir"
mkdir $eval_dir
