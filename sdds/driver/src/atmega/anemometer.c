...
#define ANEMOMETER_ISC(x) INTERNAL_CONCAT(ISC, INTERNAL_CONCAT(DRIVER_ANEMOMETER_INTERRUPT, x))
#define ANEMOMETER_INT INTERNAL_CONCAT(INT, DRIVER_ANEMOMETER_INTERRUPT)
#define ANEMOMETER_VECTOR INTERNAL_CONCAT(INT, INTERNAL_CONCAT(DRIVER_ANEMOMETER_INTERRUPT, _vect))
#define ANEMOMETER_INTF INTERNAL_CONCAT(INTF, DRIVER_ANEMOMETER_INTERRUPT)
#define DRIVER_ANEMOMETER_PORTR INTERNAL_CONCAT(PORT, DRIVER_ANEMOMETER_PORT)
#define DRIVER_ANEMOMETER_PORTVAL INTERNAL_CONCAT(P, INTERNAL_CONCAT(DRIVER_ANEMOMETER_PORT, DRIVER_ANEMOMETER_PIN))
...
static void anemometer_callback(void)
{
	...
	time_passed = g_anemometer_callback_calls * DRIVER_ANEMOMETER_CALLBACK_PERIOD;
	...
	g_anemometer_values[2] = INTERNAL_MIN(UINT16_MAX, (g_anemometer_ticks[2] * 240) / DRIVER_ANEMOMETER_CALLBACK_PERIOD);
	g_anemometer_ticks[2] = 0;
}

rc_t anemometer_init(void)
{
	DRIVER_ANEMOMETER_PORTR |= _BV(DRIVER_ANEMOMETER_PORTVAL);
	...
	DRIVER_ANEMOMETER_EICR |= _BV(ANEMOMETER_ISC(1)) | _BV(ANEMOMETER_ISC(0));
#else
	// negative edge triggered
	DRIVER_ANEMOMETER_EICR |= _BV(ANEMOMETER_ISC(1));
	DRIVER_ANEMOMETER_EICR &= ~_BV(ANEMOMETER_ISC(0));
#endif
