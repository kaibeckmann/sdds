...
int main()
{
    ...
    //  Test with QOS RELIABILITY and kind RELIABLE
    ...
    //  Enqueue
    rc = sdds_History_enqueue_buffer (&history, &buffer1, 0);
    assert (rc == SDDS_RT_OK);
    rc = sdds_History_enqueue_buffer (&history, &buffer2, 1);
    assert (rc == SDDS_RT_FAIL);
