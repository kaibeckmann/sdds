...
{
  ASSERT(HAL_RTIMER_STOPPED_MODE == __rtimer.mode, HALMACISR_RTIMER_ALREADY_STARTED);
...
ISR(TRX24_RX_END_vect)
{
  BEGIN_MEASURE
  phyRxEndInterrupt();
  END_MEASURE(HALMACISR_TRX24_RX_END_TIME_LIMIT)
}
...
ISR(TRX24_TX_END_vect)
{
  BEGIN_MEASURE
  phyTxEndInterrupt();
  END_MEASURE(HALMACISR_TRX24_TX_END_TIME_LIMIT)
}
...
ISR(BAT_LOW_vect)
{
  BEGIN_MEASURE
  phyBatLowInterrupt();
  END_MEASURE(HALMACISR_BAT_LOW_TIME_LIMIT)
}
#endif // _RF_BAT_MON_
