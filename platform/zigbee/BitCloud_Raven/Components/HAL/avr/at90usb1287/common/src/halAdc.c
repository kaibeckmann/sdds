...
******************************************************************************/
#if F_CPU == 4000000
  PROGMEM_DECLARE(const uint8_t halAdcDevider[5]) = {2, 3, 4, 5, 6};
#elif F_CPU == 8000000
  PROGMEM_DECLARE(const uint8_t halAdcDevider[5]) = {3, 4, 5, 6, 7};
#endif
...
void  halOpenAdc(HAL_AdcParams_t *param)
{
  ...
  uint8_t tmp;
  memcpy_P(&tmp, &(halAdcDevider[param->sampleRate]), 1);
  ADCSRA = tmp | (1 << ADEN);
}
