/**************************************************************************//**
  ...
  \internal
   ...
   Last change:
    $Id: nwkRouteRequest.h 12570 2010-09-24 20:25:34Z mgekk $
 ******************************************************************************/
...
#define  NWK_INITIAL_MANYTOONE_ROUTE_REQ_TX_PARAMETERS \
  {NWK_TX_DELAY_INITIAL_MANYTOONE_ROUTE_REQUEST, NULL, NULL, true}
...
typedef struct PACK _NwkRouteRequestOptions_t
{
  LITTLE_ENDIAN_OCTET(5, (
    ...
    /** The many-to-one field shall have one of the non-reserved values shown in
     *  ZigBee spec r18, Table 3.41 and type NwkRReqManyToOneFlag_t.*/
...
typedef struct PACK _NwkRouteRequestPayload_t
{
  ...
  /** The route request identifier is an 8-bit sequence number for route requests
   *  and is incremented by 1 every time the NWK layer on a particular device
   ** issues a route request. ZigBee spec r18, 3.4.1.3.2, page 321. */
