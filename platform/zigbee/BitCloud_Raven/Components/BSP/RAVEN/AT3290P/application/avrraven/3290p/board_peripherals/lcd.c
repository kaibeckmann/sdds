...
int lcd_num_clr_all(void)
{
    ...
    for (int i=0;i<4;++i){
        for (int j=0;j<LCD_ALPHA_DIGIT_COUNT;++j){
            lcd_data[PROGMEM_READ_BYTE(&seg_inf[j])&0x1F] &= ~((PROGMEM_READ_BYTE(&seg_inf[j])>>5)<<(i*2));
        }
    }
    return 0;
}
...
int lcd_num_clr(int8_t digit)
{
    ...
    for (int j=0;j<LCD_ALPHA_DIGIT_COUNT;++j){
        lcd_data[PROGMEM_READ_BYTE(&seg_inf[j])&0x1F] &= ~((PROGMEM_READ_BYTE(&seg_inf[j])>>5)<<(digit*2));
    }
...
void lcd_symbol_set_group(lcd_symbol_index_t index, int count)
{
    ...
    for( int i=index; i<index + count; ++i){
        lcd_symbol_set(PROGMEM_READ_BYTE(&lcd_symbol_chart[i]));
    }
}
...
void lcd_symbol_clr_group(lcd_symbol_index_t index, int count)
{
    ...
    for( int i=index; i<index + count; ++i){
        lcd_symbol_clr(PROGMEM_READ_BYTE(&lcd_symbol_chart[i]));
    }
}
...
static int lcd_num_print(uint16_t numb, bool negative, lcd_padding_t padding)
{
    ...
    for (int i=0;i<4;/**/) {
        ...
        lcd_nmb_print_dig(PROGMEM_READ_BYTE(&seg_map[(numb&(0xF<<4*i))>>4*i]), i);

        // if rest of number is zero
        if (++i<4) {
            if (numb >> 4*i == 0) {
                if (negative == true) { // print a 'minus' in the next digit field
                    lcd_nmb_print_dig(PROGMEM_READ_BYTE(&seg_map[(padding == LCD_NUM_PADDING_ZERO) ? LCD_SEV_SEG_INDEX_0 : LCD_SEV_SEG_INDEX_MINUS]), i++);
                ...
                while (i<4){
                    lcd_nmb_print_dig(PROGMEM_READ_BYTE(&seg_map[(padding == LCD_NUM_PADDING_ZERO) ? LCD_SEV_SEG_INDEX_0 : LCD_SEV_SEG_INDEX_SPACE]), i++);
                }
            }
        } else {
            if (negative == true) {
...
static void lcd_nmb_print_dig(uint8_t val, int dig)
{
    ...
    if (lcd_num_disable_mask & (1<<dig)){
        val = PROGMEM_READ_BYTE(&seg_map[LCD_SEV_SEG_INDEX_SPACE]);
    }
    
    for (int j=0;j<LCD_ALPHA_DIGIT_COUNT;++j){
        if (val & (1<<j)) {
            lcd_data[PROGMEM_READ_BYTE(&seg_inf[j])&0x1F] |= (PROGMEM_READ_BYTE(&seg_inf[j])>>5)<<(dig*2);
        }
        else {
            lcd_data[PROGMEM_READ_BYTE(&seg_inf[j])&0x1F] &= ~((PROGMEM_READ_BYTE(&seg_inf[j])>>5)<<(dig*2));
        }
